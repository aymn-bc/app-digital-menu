  ...
  persist(
    (set) => ({
      ...
      setPendingOrders: (n: number | ((prev: number) => number)) => {
        set((state) => ({ pendingOrders: typeof n === 'function' ? (n as any)(state.pendingOrders) : n }))
      },
      setLanguage: (l: string) => set({ language: l }),
      setThemeVars: (vars: Record<string, string>) => set({ themeVars: vars }),
      setSelectedRestaurant: (id: string | null) => set({ selectedRestaurantId: id }),
    }),
    {
      name: 'app-storage',
      partialize: (state) => ({ selectedRestaurantId: state.selectedRestaurantId, language: state.language }),
    }
  )
)
...
export const selectSetSelectedRestaurant = (s: AppState) => s.setSelectedRestaurant
